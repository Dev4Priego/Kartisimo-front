      ...
      <transition name="slide-down" class="mb-3">
        ...
        >
          ...
          <div class="mt-3 d-flex justify-content-end align-items-center gap-2">
            <span class="me-auto text-muted" v-if="file"
            ...
            <transition name="fade">
              <div
                v-if="isLoading"
                class="bg-success rounded d-flex justify-content-center align-items-center"
...
const cargarExistenciasInventario = async () => {
  try {
    const res = await fetch(
      proxy.$serverIP + "api/InventarioLlanta/existenciasInventario",
    );
    const data = await res.json();
    llantas.value = data.map((llanta) => {
      const anchura =
        llanta.anchura && llanta.anchura !== 0 ? llanta.anchura : "";
      const perfil = llanta.perfil && llanta.perfil !== 0 ? llanta.perfil : "";
      const rin = llanta.rin && llanta.rin !== 0 ? llanta.rin : "";
      const runflat =
        llanta.runflat && llanta.runflat !== "" ? llanta.runflat : "";
      const carga = llanta.carga && llanta.carga !== 0 ? llanta.carga : "";
      const velocidad =
        llanta.velocidad && llanta.velocidad !== "N/A" ? llanta.velocidad : "";
      llanta.precio = Number(llanta.precio).toLocaleString("es-MX", {
        style: "currency",
        currency: "MXN",
      });
      llanta.costo = Number(llanta.costo).toLocaleString("es-MX", {
        style: "currency",
        currency: "MXN",
      });
      let medida = "";
      if (llanta.medida) {
        ...
        if (carga || velocidad)
          medida += ` ${carga ? carga : ""}${velocidad ? velocidad : ""}`;
        medida = medida.trim();
      }
...
const cargarListaAlmacen = async () => {
  try {
    const res = await fetch(
      proxy.$serverIP + "api/InventarioLlanta/mostrarAlmacen",
    );
    const data = await res.json();
...
const llantasFiltradas = computed(() => {
  ...
  return llantas.value.filter((llanta) => {
    ...
    const coincideAlmacen =
      !almacen || llanta.nombreAlmacen.toLowerCase() === almacen;

    return (coincideMedida || coincideTodoJunto) && coincideAlmacen;
  });
});

watch([busquedaCodigoDesc, almacenSeleccionado], () => {
  tableKey.value++;
});
...
const filesByBrand = reactive(Object.fromEntries(brands.map((b) => [b, []])));
...
async function submit() {
  ...
  try {
    ...
    if (ListaLLantas.value.length != 0) {
      // llantas nuevas
