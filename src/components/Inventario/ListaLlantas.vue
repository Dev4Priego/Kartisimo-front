      ...
      >
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="container-fluid py-3">
              <div class="row mb-3">
                ...
                <div class="col-md-4">
                  <label for="FiltroMarca" class="mb-1"
                ...
                <div class="mb-3">
                  <label class="form-label"
                    >Asignar marca a resultados filtrados</label
                  >
              ...
              <div class="table">
                <table class="table table-striped table-hover align-middle">
                  ...
                  <tbody>
                    <tr
                      v-for="(llanta, index) in llantasPaginadas"
                      :key="`llanta-row-${index}`"
                    >
                      <!--Codigo-->
                      <td class="text-center">
                        <input
                          type="text"
                          class="form-control"
                          v-model="llanta.codigo"
                          @input="validateField(llanta, 'codigo', index)"
                          @blur="validateField(llanta, 'codigo', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'codigo'),
                          }"
                        />
                        <small
                          v-if="hasFieldError(index, 'codigo')"
                          class="error-msg"
                          >{{ getFieldError(index, "codigo") }}</small
                        >
                      </td>
                      <!--Desc-->
                      ...
                      <td>
                        <select
                          ...
                          id="marca"
                          class="form-select"
                          v-model.number="llanta.marca"
                          @change="validateField(llanta, 'marca', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'marca'),
                          }"
                        >
                        ...
                        <small
                          v-if="hasFieldError(index, 'marca')"
                          class="error-msg"
                          >{{ getFieldError(index, "marca") }}</small
                        >
                      </td>
                      <!--Modelo-->
                      <td>
                        <input
                          type="text"
                          class="form-control"
                          v-model="llanta.modelo"
                          @input="validateField(llanta, 'modelo', index)"
                          @blur="validateField(llanta, 'modelo', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'modelo'),
                          }"
                        />
                        <small
                          v-if="hasFieldError(index, 'modelo')"
                          class="error-msg"
                          >{{ getFieldError(index, "modelo") }}</small
                        >
                      </td>
                      <!--Carga-->
                      <td>
                        <input
                          ...
                          v-model.number="llanta.carga"
                          @input="validateField(llanta, 'carga', index)"
                          @blur="validateField(llanta, 'carga', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'carga'),
                          }"
                        />
                        <small
                          v-if="hasFieldError(index, 'carga')"
                          class="error-msg"
                          >{{ getFieldError(index, "carga") }}</small
                        >
                      </td>
                      <!--subCarga (Dual) Este campo es opcional-->
                      <td>
                        <input
                          type="number"
                          class="form-control sin-flechas"
                          v-model.number="llanta.subCarga"
                      
                          :class="{
                            'input-error': hasFieldError(index, 'subCarga'),
                          }"
                        />
                        <small
                          v-if="hasFieldError(index, 'subCarga')"
                          class="error-msg"
                          >{{ getFieldError(index, "subCarga") }}</small
                        >
                      </td>
                      <!--Vel-->
                      <td>
                        <input
                          type="text"
                          class="form-control"
                          v-model="llanta.velocidad"
                          @input="validateField(llanta, 'velocidad', index)"
                          @blur="validateField(llanta, 'velocidad', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'velocidad'),
                          }"
                        />
                        <small
                          v-if="hasFieldError(index, 'velocidad')"
                          class="error-msg"
                          >{{ getFieldError(index, "velocidad") }}</small
                        >
                      </td>
                      <!--ancho-->
                      <td>
                        <input
                          type="number"
                          class="form-control sin-flechas"
                          v-model.number="llanta.anchura"
                          @input="validateField(llanta, 'anchura', index)"
                          @blur="validateField(llanta, 'anchura', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'anchura'),
                          }"
                        />
                        <small
                          v-if="hasFieldError(index, 'anchura')"
                          class="error-msg"
                          >{{ getFieldError(index, "anchura") }}</small
                        >
                      </td>
                      <!--perfil-->
                      <td>
                        <input
                          type="number"
                          class="form-control sin-flechas "
                          v-model.number="llanta.perfil"
                          @input="validateField(llanta, 'perfil', index)"
                          @blur="validateField(llanta, 'perfil', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'perfil'),
                          }"
                        />
                        <small
                          v-if="hasFieldError(index, 'perfil')"
                          class="error-msg"
                          >{{ getFieldError(index, "perfil") }}</small
                        >
                      </td>
                      <!--rin-->
                      <td>
                        <input
                          type="number"
                          class="form-control sin-flechas"
                          v-model.number="llanta.rin"
                          @input="validateField(llanta, 'rin', index)"
                          @blur="validateField(llanta, 'rin', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'rin'),
                          }"
                        />
                        <small
                          v-if="hasFieldError(index, 'rin')"
                          class="error-msg"
                          >{{ getFieldError(index, "rin") }}</small
                        >
                      </td>
                      <!--capas-->
                      <td>
                        <input
                          type="number"
                          class="form-control sin-flechas"
                          v-model.number="llanta.capas"
                          :disabled="!usaCapas(llanta.nomenclatura)"
                          @input="validateField(llanta, 'capas', index)"
                          @blur="validateField(llanta, 'capas', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'capas'),
                          }"
                        />
                        <small
                          v-if="hasFieldError(index, 'capas')"
                          class="error-msg"
                          >{{ getFieldError(index, "capas") }}</small
                        >
                      </td>
                      <!--runFlat-->
                      <td>
                        <select
                          name="runflat"
                          id="runflat"
                          v-model.number="llanta.rf"
                          class="form-select"
                          @change="validateField(llanta, 'rf', index)"
                          :class="{ 'input-error': hasFieldError(index, 'rf') }"
                        >
                          <option :value="0">No</option>
                          <option :value="1">Sí</option>
                        </select>
                        <small
                          v-if="hasFieldError(index, 'rf')"
                          class="error-msg"
                          >{{ getFieldError(index, "rf") }}</small
                        >
                      </td>
                      <!--cantidad-->
                      <td>
                        <input
                          type="number"
                          class="form-control sin-flechas"
                          v-model.number="llanta.cantidad"
                      ...
                      <td>
                        <input
                          type="number sin-flechas"
                          class="form-control sin-flechas"
                          v-model.number="llanta.costo"
                          step="any"
                      ...
                      <td>
                        <select
                          class="form-select"
                          v-model.number="llanta.nomenclatura"
                          @change="validateField(llanta, 'nomenclatura', index)"
                          :class="{
                            'input-error': hasFieldError(index, 'nomenclatura'),
                          }"
                        >
                        ...
                        <small
                          v-if="hasFieldError(index, 'nomenclatura')"
                          class="error-msg"
                          >{{ getFieldError(index, "nomenclatura") }}</small
                        >
                      </td>
          ...
          <div class="modal-footer">
            ...
            <button class="btn btn-success" @click="GuardarLlantas">
              Cargar Lista
            </button>
          </div>
        </div>
...
const getErrorKey = (index, field) => `${index}-${field}`;
const getFieldError = (index, field) =>
  errores[getErrorKey(index, field)] || null;
const hasFieldError = (index, field) => Boolean(getFieldError(index, field));

const validateField = (llanta, field, index) => {
  const key = getErrorKey(index, field);
  const valor = llanta?.[field];
  ...
  switch (field) {
    case "codigo":
      error = !String(valor ?? "").trim() ? "El código es obligatorio" : null;
      break;
    case "marca":
      error =
      ...
      break;
    case "modelo":
      error = !String(valor ?? "").trim() ? "El modelo es obligatorio" : null;
      break;
    case "carga":
      error = Number(valor ?? 0) <= 0 ? "La carga es obligatoria" : null;
      break;
    case "subCarga":
      if (
        valor !== null &&
        ...
        error = "La carga dual debe ser mayor a 0";
      }
      break;
    case "velocidad":
      error =
        !String(valor ?? "").trim() || String(valor).toUpperCase() === "N/A"
          ? "La velocidad es obligatoria"
          : null;
      break;
    case "anchura":
      error = Number(valor ?? 0) <= 0 ? "La anchura es obligatoria" : null;
      break;
    case "perfil":
      if ([1, 2, 5, 6].includes(nomenclatura) && Number(valor ?? 0) <= 0) {
        error = "El perfil es obligatorio";
      }
      break;
    case "rin":
      if (
        [1, 2, 3, 4, 6, 7].includes(nomenclatura) &&
        Number(valor ?? 0) <= 0
      ) {
        error = "El rin es obligatorio";
      }
      break;
    case "capas":
      if (usaCapas(nomenclatura) && Number(valor ?? 0) <= 0) {
        error = "Las capas son obligatorias";
      }
      break;
    case "rf":
      error =
    ...
    case "nomenclatura":
      error = !Number(valor || 0) ? "Selecciona la nomenclatura" : null;
      break;
    default:
      error = null;
      break;
...
const validateRow = (llanta, index) => {
  const fields = [
    "codigo",
    "marca",
    "modelo",
  ...
  return fields
    .map((field) => validateField(llanta, field, index))
    .filter(Boolean);
};
...
watch(
  () => props.llantas,
  () => {
    props.llantas.forEach((llanta, index) => {
      validateRow(llanta, index);
    });
  },
...
const nomenclaturas = [
  ...
  {
    value: 6,
    label: "Metrica con rin por guion",
    example: "145/70-6, 145/70-6-6C",
  },
...
const formatStoredDecimal = (value, forceTwoDecimals = false) => {
  ...
  const shouldNormalize =
    Number.isInteger(number) && (forceTwoDecimals || Math.abs(number) >= 1000);
...
const usaCapas = (nomenclatura) =>
  [2, 4, 5, 6, 7].includes(Number(nomenclatura || 0));
...
const obtenerCapas = (llanta) => {
  const directa = toNumber(
    llanta.capas ?? llanta.Capas ?? llanta.capa ?? llanta.Capa,
  );
...
const buildMedida = (llanta) => {
  ...
  if (nomenclatura === 2) {
    if (!perfil) return "";
    return `${formatStoredDecimal(ancho)}X${formatStoredDecimal(
      perfil,
      true,
    )}R${formatRin(rin)}${buildCapas(llanta)}`;
  }

  if (nomenclatura === 3) {
    return `${formatStoredDecimal(
      ancho,
      ancho >= 1000 || (ancho >= 500 && rin >= 1000),
    )}R${formatRin(rin)}`;
  }
  ...
  if (nomenclatura === 4) {
    ...
    return `${formatStoredDecimal(ancho, ancho >= 1000)}${separador}${formatRin(
      rin,
    )}${buildCapas(llanta)}`;
  }
  ...
  if (nomenclatura === 7) {
    return `${formatStoredDecimal(ancho, ancho >= 100)}X${formatRin(
      rin,
    )}${buildCapas(llanta)}`;
  }
...
const buildRango = (llanta) => {
  ...
  const velocidad = String(llanta.velocidad || "")
    .trim()
    .toUpperCase();
...
const EliminarRegistro = (item) => {
  const index = props.llantas.indexOf(item);
  if (index === -1) return;

  props.llantas.splice(index, 1);

  Object.keys(errores).forEach((key) => delete errores[key]);
  props.llantas.forEach((llanta, rowIndex) => validateRow(llanta, rowIndex));

  const paginas = Math.max(1, Math.ceil(llantasFiltradas.value.length / pageSize.value));
  if (page.value > paginas) page.value = paginas;
};
...
const ValidateList = () => {
  ...
  props.llantas.forEach((llanta, index) => {
    ...
    const erroresFila = validateRow(llanta, index);
...
const GuardarLlantas = async () => {
  ...
  if (!props.almacen) {
    Swal.fire(
      "Error",
      "No se pudo identificar el almacen de la lista.",
      "error",
    );
    return;
  }
